...
build: clean
	...
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -o ./build/sentimentd.linux-amd64 -ldflags='-X main.Version=$(VERSION) -extldflags "-static"' .
	CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -a -o ./build/sentimentd.darwin-amd64 -ldflags='-X main.Version=$(VERSION) -extldflags "-static"' .
